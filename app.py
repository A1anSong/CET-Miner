...
async def startTrading(apiKey, secretKey):
    ...
    cet_amount = float('%.2f' % balance['CET']['free'])
    if cet_amount > 1:
    ...
    while trading_fee_sum < TARGET_TRADING_FEE:
        ...
        if sc_amount * trading_price < usdt_amount * 0.9:
            buy_sc_order = await coinex.create_market_buy_order('SC/USDT', usdt_amount * 0.4)
            trading_fee_sum += buy_sc_order['fee']['cost'] * float(buy_sc_order['info']['avg_price'])
            print('计入市价买单手续费，trading_fee_sum', trading_fee_sum)
        elif sc_amount * trading_price * 0.9 > usdt_amount:
            sell_sc_order = await coinex.create_market_sell_order('SC/USDT', sc_amount * 0.4)
